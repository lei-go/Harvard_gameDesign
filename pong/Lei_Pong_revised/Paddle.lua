...
function Paddle:init(x, y, width, height, dy, upCommand, downCommand)
    self.x = x
    self.y = y
    self.width = width
    self.height = height
...
function Paddle:update(dt)
    ...
    if love.keyboard.isDown(self.upCommand) then
        self.dy = -PADDLE_SPEED
    elseif love.keyboard.isDown(self.downCommand) then
        self.dy = PADDLE_SPEED
    else
        self.dy = 0
    end
